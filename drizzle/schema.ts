...
// ==================== INTERACTIVE CLASSROOM (OpenMAIC) ====================

/**
 * Classrooms table — interactive classroom sessions generated by AI.
 * Each classroom contains multiple scenes (slides, quizzes, discussions).
...
export const classrooms = mysqlTable("classrooms", {
  ...
  /** Teacher agent config (JSON: { name, avatar, personality, systemPrompt }) */
  teacherConfig: json("teacherConfig"),
  /** Student agents config (JSON array: [{ id, name, avatar, personality }]) */
  studentAgents: json("studentAgents"),
  /** Error message if generation failed */
  errorMessage: text("errorMessage"),
  /** Source learning material ID (optional, if generated from material) */
...
/**
 * Classroom scenes table — individual scenes within a classroom.
 * Types: slide (PPT presentation), quiz (assessment), discussion (multi-agent).
 */
export const classroomScenes = mysqlTable("classroom_scenes", {
  id: int("id").autoincrement().primaryKey(),
  /** Parent classroom ID */
  classroomId: int("classroomId").notNull(),
  /** Scene order index (1-based) */
  sceneIndex: int("sceneIndex").notNull(),
  /** Scene type: slide, quiz, discussion */
  sceneType: mysqlEnum("sceneType", ["slide", "quiz", "discussion"]).notNull(),
  /** Scene title */
  title: varchar("title", { length: 256 }).notNull(),
  /** Scene description / teaching objective */
  description: text("description"),
  /** Key points covered (JSON array of strings) */
  keyPoints: json("keyPoints"),
  /** Estimated duration in seconds */
  estimatedDuration: int("estimatedDuration").default(120).notNull(),
  /**
   * Slide content (JSON) — for slide type:
   * { background: { type, color }, elements: [{ id, type, left, top, width, height, content, ... }] }
   */
  slideContent: json("slideContent"),
  /**
   * Teaching actions (JSON array) — for slide type:
   * [{ type: "speech"|"spotlight"|"discussion", content/params }]
   */
  actions: json("actions"),
  /**
   * Quiz questions (JSON array) — for quiz type:
   * [{ id, type: "single"|"multiple"|"short_answer", question, options, answer, analysis, points }]
   */
  quizQuestions: json("quizQuestions"),
  /**
   * Discussion config (JSON) — for discussion type:
   * { topic, prompt, agentId }
   */
  discussionConfig: json("discussionConfig"),
  /** Whether this scene has been generated */
  isGenerated: boolean("isGenerated").default(false).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});

export type ClassroomScene = typeof classroomScenes.$inferSelect;
export type InsertClassroomScene = typeof classroomScenes.$inferInsert;

/**
 * Classroom messages table — multi-agent discussion messages.
 * Records all messages from teacher, student agents, and user during discussions.
 */
export const classroomMessages = mysqlTable("classroom_messages", {
  id: int("id").autoincrement().primaryKey(),
  /** Parent classroom ID */
  classroomId: int("classroomId").notNull(),
  /** Scene ID where this message was sent */
  sceneId: int("sceneId"),
  /** Sender type: teacher, student, user */
  senderRole: mysqlEnum("senderRole", ["teacher", "student", "user"]).notNull(),
  /** Sender agent ID (e.g. "teacher-1", "student-1", or user ID) */
...
export type InsertClassroomMessage = typeof classroomMessages.$inferInsert;

/**
 * Student classroom progress — tracks student's progress through a classroom.
 */
export const studentClassroomProgress = mysqlTable("student_classroom_progress", {
  id: int("id").autoincrement().primaryKey(),
  /** Student user ID */
  userId: int("userId").notNull(),
  /** Classroom ID */
  classroomId: int("classroomId").notNull(),
  /** Current scene index (0 = not started) */
  currentSceneIndex: int("currentSceneIndex").default(0).notNull(),
  /** Quiz scores (JSON: { sceneId: { score, total, answers } }) */
