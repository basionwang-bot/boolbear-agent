...
- [x] 后端：创建语音转文字 tRPC API（音频上传 → S3 → Whisper 转写）
- [x] 前端：创建 VoiceRecorder 录音组件（麦克风按钮、录音动画、计时器、取消功能）
- [x] 前端：集成录音组件到对话页面输入框左侧
- [x] 前端：识别结果自动填入输入框
- [x] 编写 vitest 测试（160 个测试全部通过）

## 小熊语音回复（TTS 文字转语音）

- [x] 后端：创建 tts.ts 核心函数（调用 Forge TTS API）
- [x] 后端：在 voiceRouter 中添加 TTS 端点（文字→语音→S3→返回 URL）
